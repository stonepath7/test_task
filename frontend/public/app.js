...
function renderBanking() {
  ...
  const rows = filter ? (grouped[filter] || []) : collections;
  ...
  document.getElementById('bank-txs').innerHTML = rows.length ? rows.map(c=>'<div class="bank-tx"><div class="bank-icon" style="background:rgba(59,130,246,.12);">📍</div><div class="bank-desc"><strong>'+c.plate+'</strong> · '+c.model+'<div class="bank-date">'+(c.addr||'Address needed')+' · Won '+fmtD(c.date_won)+'</div></div><div class="bank-amt">'+(c.scheduled_date?fmtD(c.scheduled_date):'TBC')+'</div></div>').join('') : '<div style="padding:12px;color:var(--text3);">No inbound movements for this route filter.</div>';
